(function () {
  ...
  function stopMarkup(s,i,route) {
    ...
    const transport=leg?`<aside class="curated-leg48"><strong>↓ ${esc(leg.mode)} · 약 ${leg.minutes}분${airport?' · 14시 터미널 목표':''}</strong><p>${esc(leg.route)}</p><a href="${esc(directions)}" target="_blank" rel="noopener">${leg.travelmode==='walking'?'도보':'대중교통'} 길찾기 ↗</a></aside>`:'';
  ...
  function render() {
    ...
    root.innerHTML=`<header><div><span class="curated-eyebrow">SEPT 05—07 / THREE DAYS</span><h3>이번 여행 추천 루트</h3></div><button type="button" data-curated-remaining ${busy?'disabled':''}>선택한 일·월 적용</button></header>
      ...
      ${day==='mon'?`<section class="curated-airport53" aria-label="14시부터 15시 공항 일정"><h4>14–15시 · 공항에서 마무리</h4><ol class="curated-timeline">${D.airportPlan.map(p=>`<li><time>${p.time}</time><div><strong>${esc(p.title)}</strong><p>${esc(p.text)}</p></div></li>`).join('')}</ol><a href="https://www.kansai-airport.or.jp/flight/route/" target="_blank" rel="noopener">공항 공식 출발 절차 ↗</a></section>`:''}
