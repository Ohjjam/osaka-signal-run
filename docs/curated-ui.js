(function () {
  ...
  const travelDate=new Date().toLocaleDateString('sv-SE',{timeZone:'Asia/Tokyo'});
  let day = travelDate>='2026-09-07'?'mon':travelDate>='2026-09-06'?'sun':'sat', selected = {sat:0,sun:0,mon:0}, ready = false, busy = false, message = '';
  ...
  function stopMarkup(s,i,route) {
    ...
    const airport=day==='mon'?D.airportDepartures?.[s.id]:null;
    const next=route.stops[i+1]||(airport?{id:'airport48'}:null),leg=next?window.OsakaLegs48?.describe(s.id,next.id,airport?.minutes||s.leg):null;
    const destination=next?(P.allItems.get(next.id)||(airport?{name:'간사이국제공항',coords:[34.4347,135.2441]}:null)):null;
    ...
    const transport=leg?`<aside class="curated-leg48"><strong>↓ ${esc(leg.mode)} · 약 ${leg.minutes}분${airport?' · 16시 터미널 목표':''}</strong><p>${esc(leg.route)}</p><a href="${esc(directions)}" target="_blank" rel="noopener">${leg.travelmode==='walking'?'도보':'대중교통'} 길찾기 ↗</a></aside>`:'';
  ...
  function render() {
    ...
    root.innerHTML=`<header><div><span class="curated-eyebrow">SEPT 05—07 / THREE DAYS</span><h3>이번 여행 추천 루트</h3></div><button type="button" data-curated-remaining ${busy?'disabled':''}>선택한 일·월 적용</button></header>
      ...
      ${day==='mon'?'<p class="curated-ticket">세 코스 모두 MEGA 돈키호테 신세카이점 필수 · 쇼핑·계산 90~120분. <strong>18:00 비행 · 16:00 터미널 도착 목표</strong>. 14:30까지 식사·구경 후 짐을 챙겨 15시 공항행. T2·많은 수하물은 30분 이상 앞당기세요. 저장된 일정은 자동으로 바뀌지 않으니 원하는 월요일 코스를 적용하세요.</p>':''}
      ...
      ${day==='mon'?`<section class="curated-airport53" aria-label="16시부터 18시 공항 일정"><h4>16–18시 · 공항에서 마무리</h4><ol class="curated-timeline">${D.airportPlan.map(p=>`<li><time>${p.time}</time><div><strong>${esc(p.title)}</strong><p>${esc(p.text)}</p></div></li>`).join('')}</ol><a href="https://www.kansai-airport.or.jp/flight/route/" target="_blank" rel="noopener">공항 공식 출발 절차 ↗</a></section>`:''}
      <p class="curated-note">${esc(route.note)}</p><p class="curated-footnote">이동·대기 포함 예상 일정 · 음식점은 현장에서 교체 가능 · 운영·좌석 상황은 방문 전 확인</p>
      <footer><button type="button" class="curated-primary" data-curated-apply ${busy?'disabled':''}>${applied?'적용 중 · 다시 적용':labels[day]+' 일정에 적용'}</button><button type="button" data-curated-map>저장한 일정 지도 ↓</button><button type="button" data-curated-all ${busy?'disabled':''}>선택한 3일 적용</button>${localStorage.getItem(BACKUP)?'<button type="button" data-curated-undo>직전 적용 되돌리기</button>':''}</footer>
  ...
  function boot() {
    ...
    root.addEventListener('click',event=>{
      ...
      if(b.hasAttribute('data-curated-remaining'))void apply(['sun','mon']);
