...
for(const name of ['curated-ui.js','curated.css','vnext.js','route-transport48.js','curated-required49.js'])html=html.replace(new RegExp(name.replace('.','\\.')+'\\?v=\\d+','g'),name+'?v=50');
...
let sw=fs.readFileSync(path.join(root,'sw.js'),'utf8').replace(/const CACHE = '[^']+';/,"const CACHE = 'osaka-guide-local-v50';");
for(const name of ['curated-ui.js','curated.css','vnext.js','route-transport48.js','curated-required49.js'])sw=sw.replace(new RegExp(name.replace('.','\\.')+'\\?v=\\d+','g'),name+'?v=50');
...
if(!sw.includes("'./route-transport48.js?v=50'"))sw=sw.replace("const SHELL = [","const SHELL = [\n  './route-transport48.js?v=50',");
if(!sw.includes("'./transport48.css?v=50'"))sw=sw.replace("const SHELL = [","const SHELL = [\n  './transport48.css?v=50',");
if(!sw.includes("'./curated-required49.js?v=50'"))sw=sw.replace('const SHELL = [',"const SHELL = [\n  './curated-required49.js?v=50',");
...
console.log('Static build complete: matching entries, revision 50, offline shell and JS syntax.');
