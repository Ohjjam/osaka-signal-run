...
for(const name of ['curated-ui.js','curated.css','vnext.js'])html=html.replace(new RegExp(name.replace('.','\\.')+'\\?v=\\d+','g'),name+'?v=48');
...
let sw=fs.readFileSync(path.join(root,'sw.js'),'utf8').replace(/const CACHE = '[^']+';/,"const CACHE = 'osaka-guide-local-v48';");
for(const name of ['curated-ui.js','curated.css','vnext.js'])sw=sw.replace(new RegExp(name.replace('.','\\.')+'\\?v=\\d+','g'),name+'?v=48');
...
if(!sw.includes("'./route-transport48.js?v=48'"))sw=sw.replace("const SHELL = [","const SHELL = [\n  './route-transport48.js?v=48',");
if(!sw.includes("'./transport48.css?v=48'"))sw=sw.replace("const SHELL = [","const SHELL = [\n  './transport48.css?v=48',");
...
console.log('Static build complete: matching entries, revision 48, offline shell and JS syntax.');
